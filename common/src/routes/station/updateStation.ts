...
Router.put('/api/v1/manufacture/master-data/station/:id', async (req: any, res: any) => {
  ...
  // var station =await Station.findByIdAndUpdate(req.params.id, { $set: { ...req.body } },{
  //   new:true
  // });

  //let //station: any = await Station.findById(req.params.id);
  //station.updatedAt = new Date();
  let station =await Station.findByIdAndUpdate(req.params.id,
    {$set: {...req.body,updatedAt:new Date()}}, {new:true});

    console.info("Station Props ",station)
