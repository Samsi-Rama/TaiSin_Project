<template>
  <div>
    <b-breadcrumb>
      ...
      <b-breadcrumb-item active>Edit Role</b-breadcrumb-item>
    </b-breadcrumb><br>

    <b-card title="Edit Role">
      <b-row style="margin-bottom:15px;">
        <b-col cols="12">
          ...
          <b-button
            ...
            :to="{name:'role-view', params: { id: id }}"
          >
            View Role
          </b-button>
        </b-col>
      ...
      <b-col>
        <div>
          <b-button
            class="float-right"
            variant="primary"
            v-if="form.name !== 'Super Admin' && form.level !== 'Super Admin'"
            @click="updateRole"
          >
            Save
          </b-button>
...
export default {
  ...
  methods: {
    ...
    updateRole() {
      this.$store
        ...
        .then(() => {
          this.$bvToast.toast('Successfully Update Role', {
            title: 'Success',
            variant: 'success',
            solid: true,
          })
  ...
  mounted() {
    setTimeout(() => {
      axios
        .get('/common/roles/' + this.id)
        .then(res => {
          this.form = res.data;
          this.domain = this.domain.map(el => {
            var domain = res.data.permission.filter(doc => {
              return doc.name === el.name
            })[0]
            if (el.name === 'Quality Control' && domain['menu']) {
              el.masterdata = el.masterdata.map(menu => {
                // Filter
                var masterdataMenu = domain['menu'].filter(doc => {
                  return doc.name === menu.name
                })[0]
